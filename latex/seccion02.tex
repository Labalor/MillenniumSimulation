...
Análogamente, antes de analizar la \textit{Figura}~\ref{fig:fig5} en la que se recogen la evolución de cada conjunto de subhalos para cada rango de masas de la \textit{Tabla}~\ref{tab:tabla2}, se presenta en la \textit{Figura}~\ref{fig:fig2} un promedio de las masas para cada rango y para cada redshift. Igualmente, las escalas son logarítmicas, con lo que la variación entre cada intervalo corresponde a un cambio en el orden de magnitud. \\

La tendencia general es similar. Para los rangos de mayor masa se observa el crecimiento jerárquico esperado, aunque menos suave que en el caso de la materia oscura, y los datos alcanzan valores del redshift mayores. Para el rango de menor masa no hay una tendencia clara, presenta cambios abruptos
